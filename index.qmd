...
Instead of simply asking: “Do people who exercise more have lower blood pressure?”

we ask: “Is exercise associated with blood pressure after accounting for differences in sodium intake?”
...
## Including a covariate increase precision
...
res <- lm(blood_pressure ~ exercise + sodium_intake, data = data) |> 
  ...
  colformat_double(j = c(2, 4), digits = 2) |> 
  colformat_double(j = c(2, 3), digits = 2) |> 
  color(i = 2, j = c(3), color = "green", part = "body")
  

res2 <- lm(blood_pressure ~ exercise, data = data) |> 
  tidy() |> 
  select(term, estimate, std.error, p.value) |>
  flextable() |>
  colformat_double(j = c(2, 4), digits = 2) |> 
  colformat_double(j = c(2, 3), digits = 2) |> 
  color(i = 2, j = c(3), color = "red", part = "body")
```
...
- Risk factors of COVID-19 [@griffith2020]

- The birth-weight paradox [@hernandez2006]

- Multigenerational mobility [@breen2018]
